            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-bold text-white">
                  {settings?.strategy === 'swing' ? '🌊 Swing Trader' : settings?.strategy || 'Bot'}
                </h2>
                <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-400">
                  {settings?.mode?.toUpperCase() || 'PAPER'}
                </span>
              </div>
              <p className="text-xs text-gray-400">Running on Railway • Auto-refreshes every 30s</p>
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gray-900/50 rounded-xl p-3">
        ...
        <div className="flex flex-wrap gap-4 text-[11px] text-gray-500">
          ...
          <span>Pairs: <strong className="text-white">{settings?.pairs?.join(', ') || 'N/A'}</strong></span>
      ...
      {positions.map((pos) => (
        <div key={pos.id} className="bg-gray-900/50 rounded-xl p-3 border border-gray-700">
          <div className="flex items-center justify-between">
            <div><div className="font-bold text-white text-sm">{pos.pair}</div><div className="text-[10px] text-gray-500">{pos.side.toUpperCase()} • {pos.strategy}</div></div>
        ...
        <div key={trade.id} className="bg-gray-900/50 rounded-xl p-3 border border-gray-700">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div><div className="font-bold text-white text-sm">{trade.pair}</div><div className="text-[10px] text-gray-500">{trade.strategy} • {trade.status}</div></div>
            </div>
            <div className="text-right">
              {trade.pnl != null ? (
